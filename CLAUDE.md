...
**Audio transcription** (`lib/active_harness/agent/transcription.rb` + `lib/active_harness/providers/audio/`) — `transcribe true`/`language "..."` class DSL switches an agent's model chain to audio transcription. Only `:openrouter` is supported (`Agent::TRANSCRIPTION_PROVIDERS`); `@input` is a path to a local audio file (format auto-detected from its extension), not free text — `normalize_input` is skipped automatically. `system_prompt` has no effect (OpenRouter ignores it for this endpoint). Synchronous, no job id/polling; upstream ~60s timeout means long recordings must be chunked by the caller. See `docs/agents/audio_transcription.md`.
