...
ActiveHarness requests can generate images natively — no `custom_llm_backend`/RubyLLM required. Enable it with `image true` and use the same `model`/fallback DSL as any other request.
...
```ruby
result = ImageRequest.call(input: "A watercolor fox in a snowy forest")
...
Image APIs take a single prompt string — there's no separate system/user message split like in chat. So if the request declares a `system_prompt`, it's **prepended** to `@input` (joined by a blank line) to form the final prompt; without one, `@input` is sent as-is.
...
ImageRequest.call(input: "A fox sitting in a snowy forest")
...
`system_prompt` accepts a plain string, a Prompt class, or a lambda — same as text requests (see [Prompts](../PROMPTS.md)) — so the base style can itself be dynamic (e.g. built from `@context`/`@params`).
...
Image calls go through the same retry/fallback chain as text requests — see [Retry Policy](retry_policy.md). Provider-specific error mapping:
...
- `format :json` has no effect on image requests — `processed` always equals `output`.
- Streaming (`token:`) is not supported for image generation; it only applies to text requests.
