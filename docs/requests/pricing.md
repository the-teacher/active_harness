...
```ruby
request = SupportRequest.call(input: "What is your return policy?")
...
The cost module is designed to never raise or interrupt a request call:
...
- Any unexpected error inside `calculate_cost` returns `nil` — the request continues normally.
