...
```ruby
class RobustRequest < ActiveHarness::Request
...
begin
  request = RobustRequest.new(input: "Question")
  request.call
...
```ruby
class LoggingRequest < ActiveHarness::Request
...
```ruby
class RetryRequest < ActiveHarness::Request
...
```ruby
def call_request_with_fallback(input)
  request = RobustRequest.new(input: input)
  request.call
...
class Ai::RequestsController < ApplicationController
  def call
    begin
      request = RobustRequest.new(input: params[:input])
      request.call
      result = request.result
