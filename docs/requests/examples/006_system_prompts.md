...
How to create and use system prompts to control request behavior.
...
# Use the prompt in a request
class TeacherRequest < ActiveHarness::Request
...
When ActiveHarness resolves a system prompt class it injects the full request state before calling `#call`. All of the following are readable in any prompt class:
...
| `@config` | `Hash` | Request class-level DSL config (read-only) |
...
# default
SummaryRequest.call(input: article)

# override at call time
SummaryRequest.call(input: article, params: { max_length: 50 })
```
