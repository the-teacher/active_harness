...
Guards protect the request from invalid input, malicious content, and ensure output quality. Critical for production applications.

> ActiveHarness has no dedicated `guard` DSL. The examples below implement the same idea with the regular `on(:before_call)` / `on(:after_call)` hooks (see [request_hooks.md](../request_hooks.md)) plus a plain Ruby error class you define yourself.
...
class GuardedRequest < ActiveHarness::Request
...
```ruby
class InputGuardedRequest < ActiveHarness::Request
...
```ruby
class OutputGuardedRequest < ActiveHarness::Request
...
```ruby
class ContextGuardedRequest < ActiveHarness::Request
...
```ruby
begin
  request = GuardedRequest.new(input: "")
  request.call
...
```ruby
class Ai::RequestsController < ApplicationController
  def call
    begin
      request = GuardedRequest.new(
        input: params[:input],
...
```ruby
class ComprehensiveRequest < ActiveHarness::Request
