...
request = AnalysisRequest.new(input: "The product is great and very fast!")
request.call

puts request.result.processed["sentiment"]    # => "positive"
puts request.result.processed["analyzed_at"]  # => "2025-01-01T12:00:00Z"
```
...
> **Note:** `before_parse` and `after_parse` only run when `format :json` is declared on the request.
