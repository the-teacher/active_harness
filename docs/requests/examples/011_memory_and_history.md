...
Without memory each request call is stateless. Storing turns lets a request answer contextually — critical for chat and dialogue flows.

## How Memory Works

`ActiveHarness::Memory` is a standalone object. The request **does not** load or save memory automatically — that is always the caller's responsibility:

1. Define a project-level memory class with your defaults.
2. Write a concern that wires load/record into request hooks.
3. Include the concern in any request that needs memory.
4. Pass a memory object when calling the request.
...
## Step 3 — Include in a request

`app/ai/requests/chat_request.rb`

```ruby
class ChatRequest < ActiveHarness::Request
...
ChatRequest.call(input: "Hello! My name is Alice.", memory: memory)
ChatRequest.call(input: "What is my name?",         memory: memory)
# => "Your name is Alice."
```
...
Every request automatically looks up the context window size for its primary (first) model from `ActiveHarness::Pricing` while resolving the system prompt. The value is stored as `@context_window` (an integer, or `nil` if the model is not in the registry) and is:

- injected into **prompt class instances** alongside `@input`, `@memory`, etc. — **not** into the request itself, so it is not available in request-level hooks (`on :before_call`, etc.)
...
```ruby
# default — 25%
ChatRequest.call(input: "Hello", memory: mem)

# more history — 40%
ChatRequest.call(input: "Hello", memory: mem, params: { history_fraction: 0.4 })

# minimal — only the very last turn fits
ChatRequest.call(input: "Hello", memory: mem, params: { history_fraction: 0.05 })
```
...
`@context_window` is only injected into prompt class instances — a request-level hook such as `on :before_call` runs on the request itself and will always see `@context_window` as `nil`. Log it from inside the prompt class instead:
...
```ruby
result = ChatRequest.call(input: "Hello", memory: mem)
...
request = ChatRequest.new(input: params[:message], context: { history: history })
request.call

session[:chat_history] = request.context[:history]
```
...
memory.turns                     # raw turns: [{ request:, response:, request:, model:, at: }]
memory.to_messages               # LLM-ready: [{ role: "user", content: … }, …]
memory.to_messages(filter: ->(turn) { turn[:request] == "ChatRequest" })
