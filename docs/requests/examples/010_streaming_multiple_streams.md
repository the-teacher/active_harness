...
How to stream both tokens and request lifecycle events simultaneously, and how to handle errors during streaming.

## Why This Is Needed

In addition to raw tokens, you can stream request lifecycle events (before_call, after_call, retry, failure) to the client. This gives full visibility into what's happening during generation — useful for sidebars, progress indicators, and debugging.
...
`source` is always `:request` when used with a standalone request. In pipelines it can also be `:tribunal` or `:pipeline`.
...
puts "Full response: #{request.result.output}"
```
...
4. **Use `_source` prefix** — prefix the source param with `_` when you don't need it (request-only context)
