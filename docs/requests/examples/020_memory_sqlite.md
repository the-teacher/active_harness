...
| `meta`       | Extra fields recorded with the turn (request name, model…) as JSON text |
...
SELECT request, response,
       json_extract(meta, '$.request') AS request,
       created_at
...
SELECT *
FROM   active_harness_memory_turns
WHERE  json_extract(meta, '$.request') = 'SupportRequest'
ORDER  BY id;
```
