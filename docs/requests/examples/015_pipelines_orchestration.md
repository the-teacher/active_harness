# 015 — Pipelines and Request Orchestration

## Topic

How to build pipelines for sequential or parallel execution of multiple requests.

## Why This Is Needed

Pipelines let you combine multiple requests to solve complex tasks. For example: analyze text → translate → format.
...
First, define the prompt classes and requests:
...
class AnalysisPipeline
  ...
  def analyze_text
    request = AnalysisRequest.new(input: @input)
    request.call
    request.result.output
  end

  def detect_sentiment
    request = SentimentRequest.new(input: @input)
    request.call
    request.result.output
  end

  def translate_text
    request = TranslationRequest.new(
      input: @input,
...
class ParallelAnalysisPipeline
  ...
  def call
    ...
    translation_promise = Concurrent::Promise.execute(executor: executor) do
      request = TranslationRequest.new(input: @input, context: { target_language: "English" })
      request.call
      request.result.output
    end
...
class ConditionalPipeline
  ...
  def get_support_response
    request = SupportRequest.new(input: @input)
    request.call
    request.result.output
  end

  def send_thank_you
    request = ThankYouRequest.new(input: @input)
    request.call
    request.result.output
  end

  def request_clarification
    request = ClarificationRequest.new(input: @input)
    request.call
    request.result.output
  end
end
...
class RobustPipeline
  ...
  def detect_sentiment(previous_result)
    request = SentimentRequest.new(input: @input, context: { previous: previous_result })
    request.call
    request.result.output
  end
