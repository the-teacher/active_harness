...
How to track request performance, collect metrics, and monitor errors.
...
```ruby
class MonitoredRequest < ActiveHarness::Request
...
# Usage
request = MonitoredRequest.new(input: "Question")
request.call
```
...
```ruby
class DatabaseMonitoredRequest < ActiveHarness::Request
...
class RequestMetric < ApplicationRecord
  self.table_name = 'request_metrics'

  scope :successful, -> { where(success: true) }
  scope :failed, -> { where(success: false) }
  scope :by_request, ->(request_class) { where(request_class: request_class) }
...
```ruby
# db/migrate/xxx_create_request_metrics.rb
class CreateRequestMetrics < ActiveRecord::Migration[7.0]
  def change
    create_table :request_metrics, id: :uuid do |t|
      t.uuid :session_id, null: false
      t.string :request_class, null: false
      t.text :input
    ...
    add_index :request_metrics, :session_id
    add_index :request_metrics, :request_class
    add_index :request_metrics, :success
    add_index :request_metrics, :timestamp
  end
end
```
...
class PrometheusRequest < ActiveHarness::Request
  ...
  @@execution_time = @@prometheus.histogram(
    :request_execution_time_seconds,
    docstring: 'Request execution time in seconds',
    labels: [:request_class, :model]
  )

  @@tokens_used = @@prometheus.counter(
    :request_tokens_used_total,
  ...
  @@errors = @@prometheus.counter(
    ...
    labels: [:request_class, :error_type]
  )

  on :after_call do |result|
    labels = { request_class: self.class.name, model: result.model.name }
  ...
  on :retry do |entry, error|
    labels = { request_class: self.class.name, error_type: error.class.name }
...
```ruby
class SentryRequest < ActiveHarness::Request
  ...
  on :after_call do |result|
    Sentry.capture_message("Request call successful", level: 'info', extra: {
  ...
  on :failure do |attempts|
    Sentry.capture_message("Request failed", level: 'error', extra: {
      request: self.class.name,
...
class Ai::MetricsController < ApplicationController
  def dashboard
    @metrics = {
      success_rate: RequestMetric.success_rate,
      average_execution_time: RequestMetric.average_execution_time,
      total_cost: RequestMetric.total_cost,
      average_tokens: RequestMetric.average_tokens,
      recent_metrics: RequestMetric.recent
    }
  ...
  def by_request
    ...
    @metrics = RequestMetric.by_request(request_class).recent

    render json: @metrics
  end

  def cost_analysis
    @cost_by_model = RequestMetric.group(:model).sum(:cost)
    @cost_by_request = RequestMetric.group(:request_class).sum(:cost)

    render json: {
      by_model: @cost_by_model,
      by_request: @cost_by_request
    }
  end
end
```
