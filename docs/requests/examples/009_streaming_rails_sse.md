...
How to stream request tokens to the browser using Rails Server-Sent Events (SSE).
...
class Ai::RequestsController < ApplicationController
  ...
  def stream_response
    ...
    run_request_with_sse(input, sse)
  ensure
    sse&.close
  end
  ...
  def run_request_with_sse(input, sse)
    request = StreamingRequest.new(
      input: input,
      token: build_token_stream(sse)
    )
    request.call
    send_final_event(sse, request.result)
...
```javascript
const eventSource = new EventSource("/ai/requests/stream_response?input=Hello");
...
2. **Always close the stream in `ensure`** — prevents connection leaks even if the request raises
...
5. **Extract private helpers** — keep the action method short; delegate SSE setup and request execution to private methods
