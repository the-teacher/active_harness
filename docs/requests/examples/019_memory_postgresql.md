...
| `namespace`  | Optional per-request isolation within a session                 |
| `request`    | User input text                                               |
| `response`   | Request output text                                             |
| `meta`       | Any extra fields recorded with the turn (request name, model…)  |
...
Same `RequestMemory` concern from example 011 works unchanged:
...
def create
  ...
  result = ChatRequest.call(input: params[:message], memory: memory)
...
ChatRequest.call(input: "Hello! I'm Alice.", memory: memory)
ChatRequest.call(input: "What's my name?",  memory: memory)
# => "Your name is Alice."
...
ActiveHarness::Memory::Postgresql.new(
  ...
  namespace:        "chat", # per-request isolation within a session
  read_only:        false,
  enabled:          true,
  async:            false,
...
memory.turns                      # raw array: [{ request:, response:, request:, model:, at: }]
memory.to_messages                # LLM-ready: [{ role: "user", … }, { role: "assistant", … }]
memory.to_messages(filter: ->(t) { t[:request] == "ChatRequest" })
...
SELECT request, response, meta->>'request' AS request, created_at
...
-- All turns from a specific request
SELECT *
FROM   active_harness_memory_turns
WHERE  meta->>'request' = 'SupportRequest'
ORDER  BY id;
```
