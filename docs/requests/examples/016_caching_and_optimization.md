...
How to cache request results to improve performance and reduce costs.
...
```ruby
class CachedRequest < ActiveHarness::Request
  ...
  def call
    ...
    puts "[CACHE MISS] Calling request..."
    super
...
# Usage
request1 = CachedRequest.new(input: "What is AI?")
request1.call
puts "Result 1: #{request1.result.output}\n"

# Second call with the same question — cache will be used
request2 = CachedRequest.new(input: "What is AI?")
request2.call
puts "Result 2: #{request2.result.output}"
```
...
class RedisRequest < ActiveHarness::Request
  ...
  def generate_cache_key
    Digest::MD5.hexdigest("request:#{self.class.name}:#{@input}:#{@context.to_json}")
  end
end

# Usage
request = RedisRequest.new(input: "Question")
request.call
...
```ruby
class RailsCachedRequest < ActiveHarness::Request
  ...
  def generate_cache_key
    "request:#{self.class.name}:#{Digest::MD5.hexdigest(@input)}"
  end
end
```
...
```ruby
class SmartCachedRequest < ActiveHarness::Request
...
# Cache invalidation
SmartCachedRequest.invalidate_cache("request:SmartCachedRequest:*")
```

## Prompt-Level Caching

```ruby
class PromptCachedRequest < ActiveHarness::Request
...
```ruby
class AnalyticsRequest < ActiveHarness::Request
...
class BatchedRequest
  ...
  def process_group(inputs)
    request = AnalysisRequest.new(input: inputs.join("\n"))
    request.call

    inputs.each_with_object({}) do |input, acc|
      acc[input] = request.result.output
    end
  end
end
```
