...
Right now, "is this request going to work?" can only be answered by actually calling it. Validation is scattered and inconsistent:
...
There's no way to ask "would this request's current config even work?" without triggering the real call path, and no single place to collect *everything* wrong with a config at once — you find issues one exception at a time, in whatever order the code happens to hit them.
...
- **empty** (network/cache failure — `docs/requests/pricing.md` already documents that a fetch/cache failure returns an empty list, not an error),
...
request.valid?              # => true/false — runs validators, makes no network calls
request.errors              # => ActiveHarness::Validation::Errors (empty if valid?)
request.errors.full_messages
# => ["provider :openai is not configured (openai_api_key is missing)"]

request.warnings            # => lower-confidence, Pricing-derived notes — never affects valid?
request.warnings.full_messages
# => ["model \"some/new-model\" is not in the Pricing registry yet — capability unverified"]

request.call if request.valid?
```
...
Proposed built-ins, run automatically for every request (no opt-in needed — same spirit as `image true` already auto-validating the model chain today):
...
- `SupportedFormatValidator` — structural, only runs for `transcribe true`/`vision true` requests
- `ModelCapabilityValidator` — Pricing-backed, only runs for `image true`/`transcribe true`/`vision true` requests; emits `errors` on a confirmed mismatch, `warnings` when Pricing has no data

Any built-in could be turned off per-request, for cases where it doesn't apply:

```ruby
class WeirdRequest < ActiveHarness::Request
...
class ProfanityValidator < ActiveHarness::Validators::Base
  def validate(errors)
    errors.add(:input, :profanity, "input contains banned terms") if BANNED_WORDS =~ @request.input.to_s
  end
end
...
```ruby
class SupportRequest < ActiveHarness::Request
  validate do |request, errors|
    errors.add(:input, :too_long, "input exceeds 4000 characters") if request.input.to_s.length > 4000
  end
end
```
...
3. **Should `Tribunal`/`Pipeline` get this too?** E.g. `tribunal.valid?` aggregating each member request's errors, `pipeline.valid?` aggregating each step's. Proposing this as a fast-follow, not part of the first cut.
