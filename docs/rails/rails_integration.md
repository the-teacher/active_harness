...
```
POST /ai/request          — single request call
...
```bash
curl -s -X POST http://localhost:3000/ai/request \
...
rails generate active_harness:request    Support
...
| `active_harness:request Name`    | `app/ai/requests/name_request.rb`       |
