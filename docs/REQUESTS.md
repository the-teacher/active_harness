...
## How to Create Your First Request in 5 Minutes
...
```ruby
class GreetingRequest < ActiveHarness::Request
...
> `name:` is only used with `provider: :custom` — it selects which custom endpoint to load from the configuration. See [Custom Provider](requests/examples/013_custom_llm_backend.md).

---

## Modifying the Model Chain at Runtime

`request.models` returns a mutable `ModelList` proxy. Use it to adjust the chain after instantiation, before calling the request.
...
request.models.prepend([{ provider: :openrouter, model: "google/gemma-4-31b-it:free" }])

# Add a last-resort fallback at the end
request.models.push([{ provider: :openrouter, model: "qwen/qwen3-coder:free" }])

# Insert a model at position 1 (after the first model)
request.models.insert(1, { provider: :openrouter, model: "mistralai/mistral-small-24b-instruct-2501:free" })

# Replace the entire chain for this request only
request.models.replace([{ provider: :openrouter, model: "meta-llama/llama-3.3-70b-instruct:free" }])
...
Define a prompt that instructs the model to return JSON, and add `format :json` to the request:
...
```ruby
class SentimentRequest < ActiveHarness::Request
...
```ruby
request = SentimentRequest.new
request.input = "I love this product!"
request.call
...
To share hooks across requests, extract them into a module and use `self.included`:
...
Include the concern into a request. Use `format :json` so the parse hooks actually fire:
...
Use in a request with `provider: :custom` and `name:` matching the registered key:
...
print "AI: "
request.call("Tell me about the water cycle.", token: ->(token) { print token; $stdout.flush })
puts

puts request.result.execution_time
```
...
```javascript
const source = new EventSource("/ai/request_stream?input=" + encodeURIComponent(input));
